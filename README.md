...
-   Left click: plays a random voice clip (stops current playback if playing)
-   Right click: record a new 3-second clip or quit
-   Icon rotation during playback (rotates through branding icons)
-   Recording countdown timer in tooltip
-   Auto-play feature: 0.1% chance every 10 minutes to play a random clip
...
1. **First launch**: The app will appear in your menu bar / system tray. Right-click and select "Say something for mum" to add your first voice clip.

2. **Recording**: When you select "Say something for mum", the app will:

    - Request microphone permission (first time only)
    - Record for exactly 3 seconds
    - Show countdown timer in tooltip (hover over icon to see "3 seconds", "2 seconds", etc.)
    - Automatically save the clip
    - Show a brief confirmation in the tooltip

3. **Playback**:

    - Left-click the tray icon to hear a random clip from your collection
    - Click "{clipCount} voices for me" in the menu to play a random clip
    - During playback, the icon rotates through branding icons every 300ms
    - Clicking again while playing will stop current clip and play a new one

4. **Auto-play**: The app checks every 10 minutes with a 0.1% chance to automatically play a random clip (very rare, but adds a surprise element)

5. **Managing clips**: Audio files are stored in your app data directory:
